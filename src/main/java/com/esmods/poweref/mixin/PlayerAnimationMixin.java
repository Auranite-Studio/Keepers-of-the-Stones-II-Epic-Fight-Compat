package com.esmods.poweref.mixin;
...
public abstract class PlayerAnimationMixin<T extends LivingEntity> {
	...
	public void setupPivot(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		...
		if (entityIn instanceof Player player_)
			...
			return;
		PowerefModPlayerAnimationAPI.PlayerAnimation animation = PowerefModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null)
			return;
	...
	public void setupAnim(T entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, CallbackInfo ci) {
		if (!master.equals("poweref")) {
			if (!PowerefModPlayerAnimationAPI.animations.isEmpty())
				PowerefModPlayerAnimationAPI.animations.clear();
			return;
		}
		PlayerModel<T> model = (PlayerModel<T>) (Object) this;
		Player player = null;
		...
		if (data.getBoolean("ResetPlayerAnimation")) {
			...
			resetModelPose(model);
			PowerefModPlayerAnimationAPI.active_animations.put(player, null);
		}
		...
		PowerefModPlayerAnimationAPI.PlayerAnimation animation = PowerefModPlayerAnimationAPI.active_animations.get(player);
		if (animation == null) {
			animation = PowerefModPlayerAnimationAPI.animations.get(playingAnimation);
			PowerefModPlayerAnimationAPI.active_animations.put(player, animation);
		}
		...
		for (Map.Entry<String, PowerefModPlayerAnimationAPI.PlayerBone> entry : animation.bones.entrySet()) {
			String boneName = entry.getKey();
			PowerefModPlayerAnimationAPI.PlayerBone bone = entry.getValue();
			ModelPart modelPart = getModelPart(model, boneName);
			if (modelPart == null)
				continue;
			// Apply rotation
			Vec3 rotation = PowerefModPlayerAnimationAPI.PlayerBone.interpolate(bone.rotations, animationProgress);
			if (rotation != null) {
			...
			Vec3 position = PowerefModPlayerAnimationAPI.PlayerBone.interpolate(bone.positions, animationProgress);
			if (position != null) {
			...
			// Apply scale
			Vec3 scale = PowerefModPlayerAnimationAPI.PlayerBone.interpolate(bone.scales, animationProgress);
			if (scale != null) {
